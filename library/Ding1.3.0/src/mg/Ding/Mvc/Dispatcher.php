...
abstract class Dispatcher implements ILoggerAware, IReflectionFactoryAware
{
    ...
    public function dispatch(Action $action, IMapper $mapper)
    {
        ...
        $preDispatch = isset($dispatchInfo[2]) ? $dispatchInfo[2] : false;
        $postDispatch = isset($dispatchInfo[3]) ? $dispatchInfo[3] : false;
        $this->_logger->debug(
        ...
        if ($preDispatch) {
            if (!method_exists($controller, $preDispatch)){
                throw new MvcException("Specified preDispatch method was not found in the controller: " . get_class($controller) . ", method " + $preDispatch);
            }
        ...
        if ($postDispatch) {
            if (!method_exists($controller, $postDispatch)) {
                throw new MvcException("Specified postDispatch method was not found in the controller: " . get_class($controller) . ", method " + $postDispatch);
            }
